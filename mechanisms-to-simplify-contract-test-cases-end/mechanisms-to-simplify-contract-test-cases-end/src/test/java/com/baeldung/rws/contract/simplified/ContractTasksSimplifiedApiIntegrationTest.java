...
public class ContractTasksSimplifiedApiIntegrationTest {
    ...
    void whenCreateNewTask_thenSuccessWithExpectedFields() throws Exception { // @formatter:off
        ...
        webClient.create("/tasks", taskJson)
            .containsFields("id", "name", "projectId", "dueDate")
            .fieldsMatch(
                entry("name", equalTo("Test - New Task 1")),
                entry("projectId", greaterThan(0)),
    ...
    void givenPreloadedData_whenGetTask_thenOk() {
        webClient.get("/tasks/1")
            .containsFields("id", "uuid", "name", "description", "dueDate", "status", "projectId");
    }

    // POST - new

    @Test
    void whenCreateNewTask_thenCreatedWithoutModifyingAssociatedProjectAndToDoStatus() throws Exception { // @formatter:off
    ...
    @Test
    void whenCreateNewTaskPointingToNonExistingProject_thenBadRequest() throws Exception { // @formatter:off
        String newTaskBody = fromResource(this.resource)
            .with("projectId", 99L)
            .build();
    ...
    @Test
    void whenCreateNewTaskPointingToNullProject_thenBadRequest() throws Exception { // @formatter:off
        String newTaskBody = fromResource(this.resource)
            .withNull("projectId")
            .build();
    ...
    @Test
    void whenCreateNewTaskWithNegativeProjectId_thenBadRequest() throws Exception { // @formatter:off
        // blank name
    ...
    void whenUpdateExistingTask_thenOkWithSupportedFieldUpdated() throws Exception { // @formatter:off
        ...
        webClient.put("/tasks/" + newId, updatedTaskBody)
            .fieldsMatch(
                ...
                entry("projectId", equalTo(2)));

        webClient.get("/projects/2")
            .fieldsMatch(
                entry("name", not(emptyOrNullString())),
    ...
    @Test
    void givenPreloadedData_whenUpdateExistingTaskPointingToNonExistingProject_thenBadRequest() throws Exception { // @formatter:off
        String updatedTaskBody = fromResource(this.resource)
            .with("projectId", 99L)
            .build();
        ...
        webClient.get("/tasks/2")
            .fieldsMatch(
                entry("name", not("Test - Task To Non Existing Project")));
    } // @formatter:on
